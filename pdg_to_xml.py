...
def main():
    ...
    notfoundfile = open('not_found.txt', 'wb')
    ...
    for pdg_line in pdg_list:
        ...
        if DEBUGCOUNT % 100 == 0:
            ...
            print "100 records processed. This includes comment lines"
        ...
        if hits:
            hits_len = len(hits)
        if hits_len < 1:            
            #count if not a comment
            if journal != None: 
                #Missing, so try again
                missing_str = missing_str + ','.join(pdg_line) + '\n'
                recs_volume_mix = recs_volume_mix + 1
            hits = move_around_letters(journal,volume,pages)
            if hits:
                hits_len = len(hits)
                print 'After second try. hits_len:' + str(hits_len) + '\n content: ' + str(hits[0])
            if hits_len < 1:                
                if journal != None: 
                    #Missing again, so quit looking
                    recs_missing = recs_missing + 1                
                    print '*Missing:' + ','.join(pdg_line) + '\n'
                    notfound_str = notfound_str + ','.join(pdg_line) + '\n'
    ...
    notfoundfile.write(notfound_str)
    notfoundfile.close()
